...
func main() {
	...
	ignoredHostsFile := flag.String("ignored-hosts-file", "ignored-hosts.txt", "File with list of hosts to be ignored")
	flag.Parse()
	if os.Getenv("NO_VERIFY") == "true" {
		*noVerify = true
	}
	if os.Getenv("ENGINE_PASSWORD") != "" {
		*enginePassword = os.Getenv("ENGINE_PASSWORD")
	}
	if os.Getenv("ENGINE_URL") != "" {
		*engineURL = os.Getenv("ENGINE_URL")
	}
	if os.Getenv("ENGINE_USER") != "" {
		*engineUser = os.Getenv("ENGINE_USER")
	}
	config := Config{Target: *target,
		...
		IgnoredHostsFile:	*ignoredHostsFile,
	}

    file, err := os.Open(config.IgnoredHostsFile)
    if err != nil {
        log.Print(err)
	}
	...
	for scanner.Scan() {     
		ignoredHosts = append(ignoredHosts, scanner.Text())
    }
...
func isIgnoredHost(host string) bool {
    set := make(map[string]struct{}, len(ignoredHosts))
    for _, s := range ignoredHosts {
        set[s] = struct{}{}
    }
